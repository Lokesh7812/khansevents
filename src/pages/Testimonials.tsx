            ...
            {videoTestimonials.map((video, index) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <h3 className="text-lg font-semibold text-[#001f4d] mb-2">{video.title}</h3>
                  <p className="text-gray-600 text-sm mb-2">
                    <strong>{video.client}</strong> • {video.event}
                  </p>
                  <p className="text-gray-700 text-sm">"{video.preview}"</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
              aria-label="Close"
              className="absolute -top-12 right-0 md:top-0 md:-right-12 text-white/80 hover:text-white transition"
              onClick={() => setModalVideo(null)}
            >
              <X size={36} />
            </button>
            <div className="relative w-auto h-[80vh] max-h-[80vh]">
              <div className="relative h-full aspect-[9/16] bg-black rounded-xl overflow-hidden shadow-2xl">
                <video
                  src={modalVideo}
                  autoPlay
                  controls
